...
char* _clave_file_tag(char* file, char* tag) {
    return string_from_format("%s:%s", file, tag);
}

// Busca una pagina en la tabla correspondiente
entrada_pagina_t* _buscar_pagina(t_memoria_interna* mem, char* file, char* tag, int num_pagina) {
...
int _cargar_pagina_mock(t_memoria_interna* mem, int query_id, char* file, char* tag, int num_pagina) {
    ...
    if (mem->marco_ocupado[marco]) {
        // En CP3: aca va a ir el flush real al Storage
...
// Funcion unica que libera clave Y valor
void _destruir_entrada_tabla(char* key, void* value) {
