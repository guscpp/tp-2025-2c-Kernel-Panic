...
storage_t* iniciar_storage(char* config_path){
    storage_t* storage = malloc(sizeof(storage_t));

    storage->logger = iniciar_logger("storage.log", "STORAGE", 1, LOG_LEVEL_INFO);

    storage->config = iniciar_config(storage->logger, config_path);

    storage->puerto = config_get_int_value(storage->config, "PUERTO_ESCUCHA");
    ...
    storage->retardo_operacion = config_get_int_value(storage->config, "RETARDO_ACCESO_OPERACION");
...
int main(int argc, char* argv[]) {
    ...
    storage_t* storage = iniciar_storage(argv[1]);

    log_info(storage->logger, "Puerto leido: %d", storage->puerto);
